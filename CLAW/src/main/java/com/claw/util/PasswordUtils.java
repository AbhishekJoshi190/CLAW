...
public class PasswordUtils {
     ...
     * Hashing is a one-way process (you can't "un-hash" it back to the password).
     * This keeps user data safe even if the database is stolen.
     */
    public static String hashPassword(String plainTextPassword) {
        // Step 1: Generate a random "Salt" to make the hash unique
        // Step 2: Combine salt + password and scramble it 10 times (default)
    ...
    public static boolean checkPassword(String plainTextPassword, String hashedPassword) {
        // Security check: If there is no hash to check against, fail immediately
        if (hashedPassword == null || !hashedPassword.startsWith("$2a$")) {
            return false;
        }
        try {
            // BCrypt.checkpw handles the complex logic of extracting the salt
